...
class UsuarioModel extends DbConfig{
  ...
  public function listAllContacts($idUser) {
    try {
      $query = $this->db->prepare("with ju as (
        SELECT id_user, nome, email, foto_perfil, (
          
            select id_mensagem as iddd from tb_mensagem inner join (
        ...
        select id_user as id_contato, nome, email, foto_perfil, mensagem as ultima_mensagem, 
        data_msg as data from tb_mensagem inner join ju on ju.ultima_mensagem = id_mensagem");
